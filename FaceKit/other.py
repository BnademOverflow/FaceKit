...
def msgUrl(ses, next=None):
    html = ses.session.get(
        "https://mbasic.facebook.com/messages" if not next else next
    ).text
...
def find_id_friend(ses, name):
    html = ses.session.get(
        "https://mbasic.facebook.com/search/people/?q={}&source=filter&isTrending=0".format(
            name
        )
    ).text
    ...
    try:
        ...
        if "removefriend.php?friend_id=" not in html:
            return
        name = parsing.getTitle(html)
        id_ = re.search(r"owner_id=(\d+)", html).group(1)
        profile_picture = parsing.to_bs4(html).find(
            "img", {"alt": lambda x: x and "profile picture" in x}
        )["src"]
...
def find_id_group(ses, name):
    html = ses.session.get(
        "https://mbasic.facebook.com/search/groups/?q={}&source=filter&isTrending=0".format(
            name
        )
    ).text
    url = parsing.parsing_href(html, "__xts__", one=True)
