from . import parsing
from .checker import check_argument, check_login
...
def friend_request(ses, next=None):
    html = ses.session.get(
        "https://mbasic.facebook.com/friends/center/requests/" if not next else next
    ).text
...
def friend_requested(ses, next=None):
    html = ses.session.get(
        "https://mbasic.facebook.com/friends/center/requests/outgoing/"
        if not next
        else next
    ).text
...
def onlineFriend(ses, next=None):
    ...
    data = parsing.to_bs4(html).find_all(
        "img",
        {
            "src": lambda x: "https://static.xx.fbcdn.net/rsrc.php/v3/ym/r/bzGumJjigJ0.png"
            in x
        },
    )
