...
def comment(ses, url, text):
    ...
    try:
        ...
        html = ses.post(
            url, data=dict(fb_dtsg=fb_dtsg, jazoest=jazoest, comment_text=text)
        ).text
    except:
        status = False
...
def react(ses, url, type="like", in_reactions_picker=True):
    ...
    try:
        if type == "haha":
            url = sorting.to_mbasic(
                data.find("a", href=lambda x: "&reaction_type=4&" in x)["href"]
            )
        elif type == "wow":
            url = sorting.to_mbasic(
                data.find("a", href=lambda x: "&reaction_type=3&" in x)["href"]
            )
        elif type == "sad":
            url = sorting.to_mbasic(
                data.find("a", href=lambda x: "&reaction_type=7&" in x)["href"]
            )
        elif type == "angry":
            url = sorting.to_mbasic(
                data.find("a", href=lambda x: "&reaction_type=8&" in x)["href"]
            )
        elif type == "care":
            url = sorting.to_mbasic(
                data.find("a", href=lambda x: "&reaction_type=1&6" in x)["href"]
            )
        elif type == "love":
            url = sorting.to_mbasic(
                data.find("a", href=lambda x: "&reaction_type=2&" in x)["href"]
            )
        else:
            url = sorting.to_mbasic(
                data.find("a", href=lambda x: "&reaction_type=1&" in x)["href"]
            )
        html = ses.get(url).text
    except:
        status = False
...
def unfriend(ses, id):
    ...
    if not url:
        ...
        html = ses.post(
            "https://mbasic.facebook.com/a/removefriend.php", data=post_data
        ).text
    return Output2(status, html, r_ses.session_number)
...
def send_msg(ses, id, msg):
    ...
    if not url:
        ...
        post_data["Send"] = "Send"
        html = ses.post(
            "https://mbasic.facebook.com/messages/send/?icm=1", data=post_data
        ).text
    return Output2(status, html, r_ses.session_number)
...
def deleteMsg(ses, url):
    ...
    try:
        url = parsing.to_bs4(html).find(
            "form", action=lambda x: "/messages/action_redirect" in x
        )["action"]
...
def leave_group(ses, id):
    status = True
    html = ses.session.get(
        "https://mbasic.facebook.com/group/leave/?group_id={}".format(id)
    ).text
    try:
        data = ses.session.current_hidden_input(index=1)
        html_ = ses.session.post(
            "https://mbasic.facebook.com/a/group/leave/?qp=0", data=data
        ).text
