...
import FaceKit
from FaceKit import Account, function
...
def getMsg():
    return random.choice(
        [
            "Please use it naturally!",
            "If spam is messaged, please!",
...
def menu2():
    ...
    if choose == 0:
        ...
        print(f"   {p}1). {w}Spam Like in Home")
        limit = select(
            1, 500, msg_error=f"   {C}> {w}min: 1, max: 500", text=f"    {C}> {w}Limit:"
        )
        ...
        data = dump(
            FaceKit.like_post_friend, args=(ses,), kwargs={"id": id_}, limit=limit
        )
        for url in data:
            function.open_url(ses, url)
            count_proses(len(data))
        ...
        data = dump(
            FaceKit.like_post_grup, args=(ses,), kwargs={"id": id_}, limit=limit
        )
        for url in data:
            function.open_url(ses, url)
            count_proses(len(data))
...
def menu3():
    ...
    if choose == 1:
        ...
        data = dump(
            FaceKit.react_post_home, args=(ses,), limit=limit, show_target=False
        )
        for url in data:
        ...
        data = dump(
            FaceKit.react_post_friend, args=(ses,), kwargs={"id": id_}, limit=limit
        )
        for url in data:
            function.react(ses, url, type=type)
            count_proses(len(data))
        ...
        data = dump(
            FaceKit.react_post_grup, args=(ses,), kwargs={"id": id_}, limit=limit
        )
        for url in data:
            function.react(ses, url, type=type)
            count_proses(len(data))
...
def menu4():
    ...
    if choose == 0:
        ...
        limit = select(
            1, 100, msg_error=f"   {C}> {w}min: 1, max: 100", text=f"    {C}> {w}Limit:"
        )
        data = dump(
            FaceKit.comment_post_home, args=(ses,), limit=limit, show_target=False
        )
        for url in data:
        ...
        if choose == 2:
            ...
            print(f"   {p}4).{w} Spam Comment in Fanspage")
        id_ = input2(
            f"   {arrow}" + ("Id" if choose != 4 else "Username") + " Target: "
        )
        msg = input2(f"   {arrow}Comment value: ")
        limit = select(
            1, 100, msg_error=f"   {C}> {w}min: 1, max: 100", text=f"    {C}> {w}Limit:"
        ...
        data = dump(
            FaceKit.comment_post_friend
            if choose == 2
            else FaceKit.comment_post_grup
            if choose == 3
            else FaceKit.comment_post_fanspage,
            args=(ses,),
            kwargs={"id" if choose != 4 else "username": id_},
            limit=limit,
        )
        for url in data:
            function.comment(ses, url, msg)
        ...
        try:
            ...
            html = ""
        if "comment_text" not in html or "mbasic." not in url:
            print(f"   {p}[!]{w} url not valid")
            enter(func=menu4)
        msg = input2(f"   {arrow}Comment value: ")
        limit = select(
            1, 30, msg_error=f"   {C}> {w}min: 1, max: 30", text=f"    {C}> {w}Limit:"
        )
        confirm_execute()
        print(w)
...
def menu5():
    ...
    if choose == 0:
        ...
        banner()
        print(
            f"   {p}1).{w} Mass Accept Request"
            if choose == 1
            else f"   {p}2).{w} Mass Reject Friend"
        )
        ...
        confirm_execute()
        data = dump(
            FaceKit.friend_requested, args=(ses,), limit=limit, show_target=False
        )
        for url in data:
            function.open_url(ses, url)
            count_proses(len(data))
        ...
        limit = select(
            1, 350, msg_error=f"   {C}> {w}min: 1, max: 350", text=f"    {C}> {w}Limit:"
        )
        confirm_execute()
...
def menu6():
    ...
    if choose == 0:
        ...
        # msg = input2(f"   {arrow}Message: ")
        limit = select(
            1, 400, msg_error=f"   {C}> {w}min: 1, max: 400", text=f"    {C}> {w}Limit:"
        )
        confirm_execute()
        ...
        if (
            "messages/thread"
            not in ses.session.get("https://mbasic.facebook.com/" + id_).text
        ):
            print(f"   {p}[!] {w} id not valid")
            enter(func=menu6)
        msg = input2(f"   {arrow}Message: ")
        limit = select(
            1, 35, msg_error=f"   {C}> {w}min: 1, max: 35", text=f"    {C}> {w}Limit:"
        )
        confirm_execute()
        print(w)
        for _ in range(limit):
